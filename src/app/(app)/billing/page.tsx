...
export default function BillingPage() {
  ...
  const navBtn = (label: string, id: "usage" | "plans", Icon: React.ElementType) => (
    ...
    >
      <Icon className="h-4 w-4 text-[#1D7BFF]" />
      {label}
    </button>
  );
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-3">
            {plans.map((plan) => {
              ...
              return (
                <Card
                  key={plan.id}
                  className={cn("flex flex-col p-6", plan.highlight && "border-[#1D7BFF]/40 bg-[#1D7BFF]/[0.04]")}
                >
                  {plan.highlight && (
                  ...
                  <ul className="mt-5 flex-1 space-y-2.5">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-2 text-sm" style={{ color: "var(--text-secondary)" }}>
                        <Check className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-[#1D7BFF]" />
                        {f}
                      </li>
                    ))}
                  </ul>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isCurrent
                        ...
                        : plan.highlight
                        ? "bg-[#1D7BFF] text-black hover:bg-[#1666E8]"
